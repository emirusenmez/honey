<?php 
	...
	if (isset($_POST['add_product'])) {
		...
		if(mysqli_num_rows($select_product_name)>0){
			...
			$insert_product = mysqli_query($conn, "INSERT INTO `products`(`name`, `price`, `product_detail`, `image`, `variation`) VALUES ('$product_name','$product_price','$product_detail','$image','$variation')") or die('query failed');
			if ($insert_product) {
	...
	if (isset($_POST['updte_product'])) {
		...
		$update_variation = $_POST['variation'];
		$update_image = $_FILES['update_image']['name'];
		$update_image_tmp_name = $_FILES['update_image']['tmp_name'];
		$update_image_folder='image/'.$update_image;

		$update_query = mysqli_query($conn, "UPDATE `products` SET `id`='$update_id',`name`='$update_name',`price`='$update_price',`product_detail`='$update_detail',`image`='$update_image',`variation`='$update_variation' WHERE id = '$update_id'") or die('query failed');
		if($update_query){
